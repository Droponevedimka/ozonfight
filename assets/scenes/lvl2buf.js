...
class lvl2buf extends Phaser.Scene {
	...
	create() {
		...
		this.add.image(951.0643, 127.9323, "allgame2", "map2");
		
		var personLogo = this.add.image(1040, 125.789444, "allgame2", "pers-log-"+this.why);
		...
		this.add.text(164.52351, 232.44955, "Ты в шаге от победы! Грядет битва с повелителем бытовухи Рутиносом — не забудь товары, которые сделают тебя сильнее:\r\n", {
    "fontFamily": "Aldrich",
    "fontSize": "40px",
    "wordWrap.width": 1600
});
		...
		var btn;
		var btnTwo;
		this.itemOne = "skat.png";
		this.itemTwo = "piano.png";
		var btn1 = this.add.image(255, 545, "updateItems", "skat.png").setInteractive( { useHandCursor: true  } );
		var btn2 = this.add.image(510, 545, "updateItems", "book.png").setInteractive( { useHandCursor: true  } );
		var btn3 = this.add.image(765, 545, "updateItems", "clock.png").setInteractive( { useHandCursor: true  } );
		var btn4 = this.add.image(1120, 545, "updateItems", "piano.png").setInteractive( { useHandCursor: true  } );
		var btn5 = this.add.image(1375, 545, "updateItems", "headphones.png").setInteractive( { useHandCursor: true  } );
		var btn6 = this.add.image(1630, 545, "updateItems", "game_pad.png").setInteractive( { useHandCursor: true  } );
		btn1.alpha = 0.25;
		btn2.alpha = 0.25;
		btn3.alpha = 0.25;
		btn4.alpha = 0.25;
		...
		this.speedItemsCustom = this.add.text(400.6542, 703.64301, "+10 speed", {
			"fontFamily": "Aldrich",
			"fontSize": "46px"
		});

		this.strongItemsCustom = this.add.text(1270.6542, 703.64301, "+10 power", {
			"fontFamily": "Aldrich",
			"fontSize": "46px"
		});
		...
		btn1.on('pointerdown', function(){
			if(btn) btn.destroy();
			btn = this.add.image(255, 545, "updateItems", "skat.png");
			this.itemOne = "skat.png";
			this.getTextSpeed('Пенни борд — Рутиносу не удрать!');
		}, this);

		btn2.on('pointerdown', function(){
			if(btn) btn.destroy();
			btn = this.add.image(510, 545, "updateItems", "book.png");
			this.itemOne = "book.png";
			this.getTextSpeed('Книга: противостоит рутине уже 4000 лет ');
		}, this);

		btn3.on('pointerdown', function(){
			if(btn) btn.destroy();
			btn = this.add.image(765, 545, "updateItems", "clock.png");
			this.itemOne = "clock.png";
			this.getTextSpeed('Браслет Mi держит тебя в форме, а врага — в страхе');
		}, this);

		btn4.on('pointerdown', function(){
			if(btnTwo) btnTwo.destroy();
			btnTwo = this.add.image(1120, 545, "updateItems", "piano.png");
			this.itemTwo = "piano.png";
			this.getTextStrong('Играй на midi-клаве AKAI и нервах противника');
		}, this);

		btn5.on('pointerdown', function(){
			if(btnTwo) btnTwo.destroy();
			btnTwo = this.add.image(1375, 545, "updateItems", "headphones.png");
			this.itemTwo = "headphones.png";
			this.getTextStrong('Наушники Beats для правильных хуков');
		}, this);

		btn6.on('pointerdown', function(){
			if(btnTwo) btnTwo.destroy();
			btnTwo = this.add.image(1630, 545, "updateItems", "game_pad.png");
			this.itemTwo = "game_pad.png";
			this.getTextStrong('Консоль Nintendo: создана, чтобы проходить боссов');
		}, this);


		this.fBtn_pers.on('pointerdown', function(){
			this.sound.play('mouse-click');
			this.scene.start("anotScen3", { why: this.why, whyName: this.whyName, imageOne: this.itemOne, imageTwo: this.itemTwo, healLvl: this.healLvl});
		}, this);
